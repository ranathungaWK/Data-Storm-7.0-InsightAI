"""
Isolated outlet-potential experiment runner.

This copy is intentionally kept outside the original pipeline outputs so the
repo's canonical files remain unchanged.

Experiments in this folder:
- Version A: peak-sales model
- Version B: market-capacity model
- Ablation: market-capacity model without outlet_size / cooler_count
- Permutation importance on the market-capacity model
- Top / bottom 50 hold-out inspection for predicted market capacity
...
REPORT_PATH = BASE_DIR / "experiment_results.md"
PERMUTATION_IMPORTANCE_PATH = OUTPUT_DIR / "market_capacity_permutation_importance.csv"
ABLATION_COMPARISON_PATH = OUTPUT_DIR / "market_capacity_ablation_comparison.csv"
TOP_BOTTOM_PATH = OUTPUT_DIR / "market_capacity_top_bottom_50.csv"
...
MODEL_B_FEATURES = LOCATION_SIGNAL_FEATURES.copy()
...
def sales_baseline_column(df: pd.DataFrame) -> str:
    ...
    raise KeyError("No monthly sales baseline column found")
...
def train_version(
    *,
    ...
    sales_col = sales_baseline_column(train_df)
    ...
    if floor_to_calendar_avg:
        obs_avg = test_df[sales_col].to_numpy() if sales_col in test_df.columns else y_test
    ...
    all_test = pd.DataFrame(
        {
            ...
            "current_sales": test_df[sales_col].to_numpy() if sales_col in test_df.columns else y_test,
    ...
    all_train = pd.DataFrame(
        {
            ...
            "current_sales": train_df[sales_col].to_numpy() if sales_col in train_df.columns else y_train,
    ...
    return {
        ...
        "final_rf_model": rf_models[0],
        "holdout_raw": X_test_raw,
        "holdout_y": y_test,
        "holdout_features": X_test_raw.assign(outlet_id=ids_test),
        "feature_columns": feature_columns,
        "sales_baseline_col": sales_col,
        "scaler": scaler,
    }
...
def segment_summary_frame(holdout_predictions: pd.DataFrame, holdout_features: pd.DataFrame) -> pd.DataFrame:
    merged = holdout_predictions.merge(
        holdout_features[["outlet_id"] + [feature for feature in SUMMARY_FEATURES if feature in holdout_features.columns]],
        on="outlet_id",
        how="left",
    )

    def build_row(segment_name: str, frame: pd.DataFrame) -> dict:
        row = {
            "segment": segment_name,
            "count": int(len(frame)),
            "predicted_market_capacity_mean": float(frame["predicted_value"].mean()),
            "current_sales_mean": float(frame["current_sales"].mean()),
            "market_gap_mean": float(frame["market_gap"].mean()),
        }
        for feature in SUMMARY_FEATURES:
            row[feature] = float(frame[feature].mean()) if feature in frame.columns else np.nan
        return row

    ordered = merged.sort_values("market_gap", ascending=False).reset_index(drop=True)
    top_50 = ordered.head(50).copy()
    bottom_50 = ordered.tail(50).copy()

    top_50.to_csv(TOP_BOTTOM_PATH.with_name("market_capacity_top_50.csv"), index=False)
    bottom_50.to_csv(TOP_BOTTOM_PATH.with_name("market_capacity_bottom_50.csv"), index=False)

    return pd.DataFrame(
        [
            build_row("top_50", top_50),
            build_row("bottom_50", bottom_50),
            build_row("holdout_all", ordered),
        ]
    )
...
def write_experiment_report(
    *,
    market_full: dict,
    market_ablation: dict,
    permutation_df: pd.DataFrame,
    segment_summary: pd.DataFrame,
    market_ranking: pd.DataFrame,
) -> None:
    report_lines = [
        ...
        "",
        "## Hold-out comparison",
        f"- Full market-capacity blend RMSE: {market_full['metrics']['rmse']:.2f}",
        f"- Full market-capacity blend R²: {market_full['metrics']['r2']:.4f}",
        f"- Ablated blend RMSE: {market_ablation['metrics']['rmse']:.2f}",
        f"- Ablated blend R²: {market_ablation['metrics']['r2']:.4f}",
        f"- R² delta (full - ablated): {market_full['metrics']['r2'] - market_ablation['metrics']['r2']:+.4f}",
        f"- RMSE delta (full - ablated): {market_full['metrics']['rmse'] - market_ablation['metrics']['rmse']:+.2f}",
        "",
        ...
        market_ranking[["outlet_id", "predicted_value", "current_sales", "market_gap"]].head(10).to_string(index=False),
        "",
    ]

    if not segment_summary.empty:
        top_row = segment_summary[segment_summary["segment"] == "top_50"].iloc[0]
        bottom_row = segment_summary[segment_summary["segment"] == "bottom_50"].iloc[0]
        report_lines += [
            "## Quick read",
            f"- Top 50 predicted capacity minus bottom 50: {top_row['predicted_market_capacity_mean'] - bottom_row['predicted_market_capacity_mean']:.2f}.",
            f"- Top 50 POI influence delta: {top_row.get('poi_influence_score', np.nan) - bottom_row.get('poi_influence_score', np.nan):.2f}.",
            f"- Top 50 saturation delta: {top_row.get('saturation_score', np.nan) - bottom_row.get('saturation_score', np.nan):.2f}.",
        ]
...
def main():
    ...
    model_b = train_version(
        name="Version B - Market Capacity",
        ...
        output_prefix="market_capacity",
        floor_to_calendar_avg=True,
    )

    ablation_features = [feature for feature in MODEL_B_FEATURES if feature not in {"outlet_size", "cooler_count"}]
    model_b_ablation = train_version(
        name="Version B1 - Market Capacity Ablation",
        ...
        groups=groups,
        include_history_derived=False,
        output_prefix="market_capacity_no_size_cooler",
        floor_to_calendar_avg=True,
    )

    perm_model = BlendPredictor(
        model_b["final_model"],
        model_b["final_rf_model"],
        model_b["scaler"],
        model_b["feature_columns"],
    )
    ...
    permutation_df = pd.DataFrame(
        {
            "feature": model_b["feature_columns"],
    ...
    holdout_market = model_b["predictions"].query("split == 'test'").copy()
    holdout_market["market_gap"] = holdout_market["predicted_value"] - holdout_market["current_sales"]
    holdout_market["market_gap_pct"] = np.round(holdout_market["market_gap"] / (holdout_market["current_sales"] + 1e-6) * 100, 1)
    holdout_market = holdout_market.sort_values("market_gap", ascending=False).reset_index(drop=True)
    holdout_market.to_csv(OUTPUT_DIR / "market_capacity_market_gap_ranking.csv", index=False)

    ablation_comparison = pd.DataFrame(
        [
            {
                "model": model_b["name"],
                "rmse": model_b["metrics"]["rmse"],
                "mae": model_b["metrics"]["mae"],
                "r2": model_b["metrics"]["r2"],
                "feature_count": len(model_b["feature_columns"]),
            },
            {
                "model": model_b_ablation["name"],
                "rmse": model_b_ablation["metrics"]["rmse"],
                "mae": model_b_ablation["metrics"]["mae"],
                "r2": model_b_ablation["metrics"]["r2"],
                "feature_count": len(model_b_ablation["feature_columns"]),
            },
        ]
    )
    ablation_comparison["rmse_delta_vs_full"] = ablation_comparison["rmse"] - float(model_b["metrics"]["rmse"])
    ablation_comparison["r2_delta_vs_full"] = ablation_comparison["r2"] - float(model_b["metrics"]["r2"])
    ablation_comparison.to_csv(ABLATION_COMPARISON_PATH, index=False)

    segment_summary = segment_summary_frame(holdout_market, model_b["holdout_features"])
    segment_summary.to_csv(OUTPUT_DIR / "market_capacity_top_bottom_summary.csv", index=False)

    write_experiment_report(
        market_full=model_b,
        market_ablation=model_b_ablation,
        permutation_df=permutation_df,
        segment_summary=segment_summary,
        market_ranking=holdout_market,
    )
    ...
    comparison = comparison.rename(
        columns={
            ...
            "predicted_value_market_capacity": "predicted_market_capacity",
    ...
    comparison["market_gap"] = comparison["predicted_market_capacity"] - comparison["current_sales"]
    ...
    logger.info("\n" + comparison[["outlet_id", "predicted_peak_sales", "predicted_market_capacity", "current_sales", "market_gap", "market_gap_pct"]].head(10).to_string(index=False))

    logger.info("\nSummary metrics")
    logger.info(f"Peak-sales blend hold-out RMSE: {model_a['metrics']['rmse']:.2f} | R²: {model_a['metrics']['r2']:.4f}")
    logger.info(f"Market-capacity blend hold-out RMSE: {model_b['metrics']['rmse']:.2f} | R²: {model_b['metrics']['r2']:.4f}")
    logger.info(f"Ablated market-capacity blend hold-out RMSE: {model_b_ablation['metrics']['rmse']:.2f} | R²: {model_b_ablation['metrics']['r2']:.4f}")
    logger.info(f"Experiment report written to {REPORT_PATH}")
